...
export default async function RestaurantPage() {
    ...
    if (articles.length === 0) {
        const dummyArticles: ArticleResource[] = Array.from({ length: 30 }).map((_, i) => ({
            id: `dummy-${i}`,
            title: `Restaurant News Topic ${i + 1}: Culinary Trends of 2026`,
            summary: "Discover the latest trends in the culinary world...",
            content: "Full content would go here...",
            image: `https://placehold.co/800x450?text=Food+Image+${i + 1}`,
            category: "Restaurant",
            country: "Global",
            status: "published",
            views_count: 120 + i * 5,
    ...
    const latestPosts = articles.slice(0, 6); // Just remixing for the bottom list
