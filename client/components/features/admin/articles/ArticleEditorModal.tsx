    ...
    const getInitialArticleData = () => {
        if (initialData) {
            return {
                ...
                author: initialData.author || 'HOMESPH NEWS',
                publishDate: (initialData.date ? initialData.date.split(' ')[0] : (initialData.publish_at ? initialData.publish_at.split(' ')[0] : new Date().toISOString().split('T')[0])),
                publishTime: (initialData.date && initialData.date.includes(' ') ? initialData.date.split(' ')[1].substring(0, 5) : '14:30'),
