import { useState } from 'react';
import { Plus, ChevronLeft, ChevronRight, Globe } from 'lucide-react';
import { CreateEventModal } from './CreateEventModal';
import { EventDetailModal } from './EventDetailModal';

interface EventCalendarProps {
  sidebarCollapsed: boolean;
  onNavigate: (page: string) => void;
}

interface CalendarEvent {
  id: number;
  title: string;
  date: string;
  time: string;
  location: string;
  details: string;
  category: string;
  country: string;
  color: string;
  bgColor: string;
  borderColor: string;
  isPublicHoliday: boolean;
}

export function EventCalendar({ sidebarCollapsed, onNavigate }: EventCalendarProps) {
  const [viewMode, setViewMode] = useState<'day' | 'week' | 'month' | 'year'>('day');
  const [currentDate, setCurrentDate] = useState(new Date(2026, 0, 29)); // Jan 29, 2026 to match Figma
  const [selectedYear, setSelectedYear] = useState(2026);
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [selectedCountry, setSelectedCountry] = useState('All Countries');
  const [selectedEvent, setSelectedEvent] = useState<CalendarEvent | null>(null);
  const [showEventDetail, setShowEventDetail] = useState(false);
  const [editingEvent, setEditingEvent] = useState<CalendarEvent | null>(null);

  // Initial events with public holidays (gray) and user events (colored)
  const [events, setEvents] = useState<CalendarEvent[]>([
    // Public Holidays - Gray color scheme
    { id: 1, title: "New Year's Day", date: '2026-01-01', time: '', location: '', details: 'Public Holiday', category: 'Holiday', country: 'Philippines', color: '#111827', bgColor: '#f1f5f9', borderColor: '#f3f4f6', isPublicHoliday: true },
    { id: 2, title: "New Year's Day", date: '2026-01-01', time: '', location: '', details: 'Public Holiday', category: 'Holiday', country: 'UAE', color: '#111827', bgColor: '#f1f5f9', borderColor: '#f3f4f6', isPublicHoliday: true },
    { id: 3, title: "New Year's Day", date: '2026-01-01', time: '', location: '', details: 'Public Holiday', category: 'Holiday', country: 'Singapore', color: '#111827', bgColor: '#f1f5f9', borderColor: '#f3f4f6', isPublicHoliday: true },
    { id: 4, title: 'Independence Day', date: '2026-06-12', time: '', location: '', details: 'Public Holiday', category: 'Holiday', country: 'Philippines', color: '#111827', bgColor: '#f1f5f9', borderColor: '#f3f4f6', isPublicHoliday: true },
    { id: 5, title: 'Independence Day', date: '2026-07-04', time: '', location: '', details: 'Public Holiday', category: 'Holiday', country: 'USA', color: '#111827', bgColor: '#f1f5f9', borderColor: '#f3f4f6', isPublicHoliday: true },
    { id: 6, title: 'Labor Day', date: '2026-05-01', time: '', location: '', details: 'Public Holiday', category: 'Holiday', country: 'Philippines', color: '#111827', bgColor: '#f1f5f9', borderColor: '#f3f4f6', isPublicHoliday: true },
    { id: 7, title: 'Christmas Day', date: '2026-12-25', time: '', location: '', details: 'Public Holiday', category: 'Holiday', country: 'Philippines', color: '#111827', bgColor: '#f1f5f9', borderColor: '#f3f4f6', isPublicHoliday: true },
    // User Events - Custom colors
    { id: 10, title: 'AI in Business Conference', date: '2026-01-29', time: '10:00 AM', location: 'Cebu City, Philippines', details: 'Conference on AI and business applications', category: 'Business', country: 'Philippines', color: '#8228e5', bgColor: '#f3e7ff', borderColor: '#e9d4ff', isPublicHoliday: false },
    { id: 11, title: 'Global Real Estate Summit', date: '2026-02-15', time: '09:00 AM', location: 'Dubai', details: 'Annual real estate conference', category: 'Conference', country: 'UAE', color: '#1447e7', bgColor: '#dbeafe', borderColor: '#93c5fd', isPublicHoliday: false },
  ]);

  const monthNames = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December'];
  const dayNames = ['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const dayNamesShort = ['SUN', 'MON', 'TUE', 'WED', 'THU', 'FRI', 'SAT'];

  const getEventsForDate = (date: Date) => {
    const dateStr = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
    let filteredEvents = events.filter(event => event.date === dateStr);
    
    if (selectedCountry !== 'All Countries') {
      filteredEvents = filteredEvents.filter(event => event.country === selectedCountry);
    }
    
    return filteredEvents;
  };

  const getDaysInMonth = (year: number, month: number) => {
    return new Date(year, month + 1, 0).getDate();
  };

  const getFirstDayOfMonth = (year: number, month: number) => {
    return new Date(year, month, 1).getDay();
  };

  const navigateDate = (direction: 'prev' | 'next') => {
    const newDate = new Date(currentDate);
    if (viewMode === 'day') {
      newDate.setDate(newDate.getDate() + (direction === 'next' ? 1 : -1));
    } else if (viewMode === 'week') {
      newDate.setDate(newDate.getDate() + (direction === 'next' ? 7 : -7));
    } else if (viewMode === 'month') {
      newDate.setMonth(newDate.getMonth() + (direction === 'next' ? 1 : -1));
    }
    setCurrentDate(newDate);
  };

  const handleEventClick = (event: CalendarEvent) => {
    setSelectedEvent(event);
    setShowEventDetail(true);
  };

  const handleDeleteEvent = (eventId: number) => {
    setEvents(events.filter(e => e.id !== eventId));
  };

  const handleCreateEvent = (newEvent: CalendarEvent) => {
    setEvents([...events, newEvent]);
    setCurrentDate(new Date(newEvent.date));
    setViewMode('day');
  };

  const handleEditEvent = (event: CalendarEvent) => {
    setEditingEvent(event);
    setShowEventDetail(false);
    setShowCreateModal(true);
  };

  const handleUpdateEvent = (updatedEvent: CalendarEvent) => {
    setEvents(events.map(e => e.id === updatedEvent.id ? updatedEvent : e));
    setEditingEvent(null);
  };

  // Day View - Pixel Perfect from Figma
  const renderDayView = () => {
    const dayEvents = getEventsForDate(currentDate);
    const dayName = dayNames[currentDate.getDay()].toUpperCase().slice(0, 3);
    const dateNum = currentDate.getDate();

    return (
      <div className="bg-white rounded-[12px] border border-[#e5e7eb] shadow-sm mt-8 max-w-full">
        {/* Date Header */}
        <div className="bg-[#f9fafb] rounded-t-[12px] border-b border-[#e5e7eb] px-6 py-8">
          <div className="flex flex-col items-center justify-center gap-2">
            <p className="text-[16px] font-semibold text-[#6b7280] tracking-[0.1px] leading-[12px]">
              {dayName}
            </p>
            <p className="text-[64px] font-bold text-[#111827] tracking-[-0.5px] leading-none">
              {dateNum}
            </p>
          </div>
        </div>

        {/* Events Section */}
        <div className="p-6">
          {dayEvents.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              <div className="w-16 h-16 mb-4 text-[#d1d5db]">
                <svg viewBox="0 0 64 64" fill="none" stroke="currentColor" strokeWidth="3">
                  <rect x="8" y="12" width="48" height="44" rx="4" strokeLinecap="round" strokeLinejoin="round"/>
                  <path d="M8 24h48M20 8v8M44 8v8" strokeLinecap="round" strokeLinejoin="round"/>
                </svg>
              </div>
              <p className="text-[16px] text-[#6b7280] tracking-[-0.5px] mb-6">
                No events scheduled for this day
              </p>
              <button
                onClick={() => setShowCreateModal(true)}
                className="px-6 py-3 bg-[#dc2626] text-white rounded-[8px] hover:bg-[#b91c1c] transition-colors text-[15px] font-medium tracking-[-0.5px]"
              >
                Add Event
              </button>
            </div>
          ) : (
            <div className="space-y-4">
              {dayEvents.map((event) => (
                <div
                  key={event.id}
                  onClick={() => handleEventClick(event)}
                  className="rounded-[12px] p-4 cursor-pointer hover:opacity-90 transition-opacity border shadow-sm"
                  style={{ 
                    backgroundColor: event.bgColor,
                    borderColor: event.borderColor
                  }}
                >
                  <div className="flex items-start justify-between gap-4">
                    {/* Event Info */}
                    <div className="flex-1 min-w-0">
                      <h3 
                        className="text-[20px] font-extrabold tracking-[-0.5px] leading-normal mb-2"
                        style={{ color: event.color }}
                      >
                        {event.title}
                      </h3>
                      <div className="flex items-center gap-4 text-[14px] font-medium tracking-[-0.5px]" style={{ color: event.color }}>
                        {/* Time */}
                        <div className="flex items-center gap-1">
                          <svg width="15" height="15" viewBox="0 0 15 15" fill="none">
                            <circle cx="7.5" cy="7.5" r="6" stroke="currentColor" strokeWidth="1.5"/>
                            <path d="M7.5 4.5v3.5l2.5 1.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"/>
                          </svg>
                          <span>{event.time || 'All Day'}</span>
                        </div>
                        {/* Location */}
                        {event.location && (
                          <div className="flex items-center gap-1">
                            <svg width="12" height="15" viewBox="0 0 12 15" fill="currentColor">
                              <path d="M6 0C2.7 0 0 2.7 0 6c0 4.5 6 9 6 9s6-4.5 6-9c0-3.3-2.7-6-6-6zm0 8c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2z"/>
                            </svg>
                            <span>{event.location}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    {/* Category Badge */}
                    <div className="bg-white rounded-[4px] px-4 py-1.5 border" style={{ borderColor: event.borderColor }}>
                      <span 
                        className="text-[14px] font-semibold tracking-[-0.5px] whitespace-nowrap"
                        style={{ color: event.color }}
                      >
                        {event.category}
                      </span>
                    </div>
                  </div>
                </div>
              ))}
            </div>
          )}
        </div>
      </div>
    );
  };

  // Week View - Pixel Perfect from Figma  
  const renderWeekView = () => {
    const startOfWeek = new Date(currentDate);
    const day = startOfWeek.getDay();
    startOfWeek.setDate(startOfWeek.getDate() - day);

    const weekDays = Array.from({ length: 7 }, (_, i) => {
      const date = new Date(startOfWeek);
      date.setDate(date.getDate() + i);
      return date;
    });

    return (
      <div className="bg-white rounded-[12px] border border-[#e5e7eb] shadow-sm mt-8 overflow-hidden">
        <div className="grid grid-cols-7">
          {weekDays.map((date, idx) => {
            const dayEvents = getEventsForDate(date);
            const isToday = date.toDateString() === new Date().toDateString();
            
            return (
              <div key={idx} className="border-r border-[#e5e7eb] last:border-r-0">
                {/* Header */}
                <div className="bg-[#f9fafb] border-b border-[#e5e7eb] px-4 py-4 text-center">
                  <p className="text-[11px] font-semibold text-[#6b7280] tracking-[-0.5px] uppercase mb-1">
                    {dayNamesShort[idx]}
                  </p>
                  <p className={`text-[24px] font-bold tracking-[-0.5px] ${isToday ? 'text-[#C10007]' : 'text-[#111827]'}`}>
                    {date.getDate()}
                  </p>
                </div>
                {/* Events */}
                <div className="bg-white min-h-[500px] p-3 space-y-2">
                  {dayEvents.map((event) => (
                    <div
                      key={event.id}
                      onClick={() => handleEventClick(event)}
                      className="rounded-[6px] border shadow-sm p-3 cursor-pointer hover:opacity-90 transition-opacity"
                      style={{ 
                        backgroundColor: event.bgColor,
                        borderColor: event.borderColor
                      }}
                    >
                      <div className="flex items-center gap-1 mb-1">
                        <span className="text-[15px]">🎉</span>
                        <p className="text-[14px] font-bold tracking-[-0.5px] leading-normal truncate" style={{ color: event.color }}>
                          {event.title}
                        </p>
                      </div>
                      <p className="text-[12px] text-[#6b7280] tracking-[-0.5px] leading-[12px]">
                        {event.time || 'All Day'}
                      </p>
                    </div>
                  ))}
                </div>
              </div>
            );
          })}
        </div>
      </div>
    );
  };

  // Month View - Pixel Perfect from Figma
  const renderMonthView = () => {
    const year = currentDate.getFullYear();
    const month = currentDate.getMonth();
    const daysInMonth = getDaysInMonth(year, month);
    const firstDay = getFirstDayOfMonth(year, month);
    
    const calendarDays = [];
    
    // Previous month days
    const prevMonth = month === 0 ? 11 : month - 1;
    const prevYear = month === 0 ? year - 1 : year;
    const prevMonthDays = getDaysInMonth(prevYear, prevMonth);
    for (let i = firstDay - 1; i >= 0; i--) {
      calendarDays.push({
        date: new Date(prevYear, prevMonth, prevMonthDays - i),
        isCurrentMonth: false
      });
    }
    
    // Current month days
    for (let i = 1; i <= daysInMonth; i++) {
      calendarDays.push({
        date: new Date(year, month, i),
        isCurrentMonth: true
      });
    }
    
    // Next month days
    const nextMonth = month === 11 ? 0 : month + 1;
    const nextYear = month === 11 ? year + 1 : year;
    const remainingDays = 42 - calendarDays.length; // 6 rows
    for (let i = 1; i <= remainingDays; i++) {
      calendarDays.push({
        date: new Date(nextYear, nextMonth, i),
        isCurrentMonth: false
      });
    }

    return (
      <div className="bg-white rounded-[12px] border border-[#e5e7eb] shadow-sm mt-8 overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-7 bg-[#f9fafb] border-b border-[#e5e7eb]">
          {dayNames.map((day) => (
            <div key={day} className="px-3 py-4 text-center border-r border-[#e5e7eb] last:border-r-0">
              <p className="text-[13px] font-medium text-[#6b7280] tracking-[-0.5px]">{day}</p>
            </div>
          ))}
        </div>
        
        {/* Calendar Grid */}
        <div className="grid grid-cols-7">
          {calendarDays.map((day, idx) => {
            const dayEvents = getEventsForDate(day.date);
            const isToday = day.date.toDateString() === new Date().toDateString();
            
            return (
              <div
                key={idx}
                onClick={() => {
                  setCurrentDate(day.date);
                  setShowCreateModal(true);
                }}
                className={`min-h-[120px] p-3 border-r border-b border-[#e5e7eb] last:border-r-0 cursor-pointer hover:bg-[#f9fafb] transition-colors ${
                  !day.isCurrentMonth ? 'bg-[#fafafa]' : ''
                }`}
              >
                {/* Date Number */}
                <div className="mb-2">
                  {isToday ? (
                    <div className="inline-flex items-center justify-center w-6 h-6 rounded-full bg-[#dc2626] text-white">
                      <p className="text-[14px] font-semibold tracking-[-0.5px]">
                        {day.date.getDate()}
                      </p>
                    </div>
                  ) : (
                    <p className={`text-[14px] font-semibold tracking-[-0.5px] ${
                      !day.isCurrentMonth ? 'text-[#9ca3af]' : 'text-[#111827]'
                    }`}>
                      {day.date.getDate()}
                    </p>
                  )}
                </div>
                
                {/* Events - Simple Pills */}
                <div className="space-y-1">
                  {dayEvents.slice(0, 3).map((event) => (
                    <div
                      key={event.id}
                      onClick={(e) => {
                        e.stopPropagation();
                        handleEventClick(event);
                      }}
                      className="h-[23px] rounded-[6px] border px-3 py-2.5 flex items-center gap-1 cursor-pointer hover:opacity-80 transition-opacity shadow-sm"
                      style={{ 
                        backgroundColor: event.bgColor,
                        borderColor: event.borderColor
                      }}
                    >
                      <span className="text-[12px]">🎉</span>
                      <p className="text-[12px] font-semibold tracking-[-0.5px] truncate leading-[normal]" style={{ color: event.color }}>
                        {event.title}
                      </p>
                    </div>
                  ))}
                  {dayEvents.length > 3 && (
                    <p className="text-[10px] text-[#3b82f6] tracking-[-0.5px] pl-1">
                      +{dayEvents.length - 3} more
                    </p>
                  )}
                </div>
              </div>
            );
          })}
        </div>
      </div>
    );
  };

  // Year View - Pixel Perfect from Figma
  const renderYearView = () => {
    const months = Array.from({ length: 12 }, (_, i) => i);

    return (
      <div className="grid grid-cols-3 gap-[16px] mt-8">
        {months.map((monthIdx) => {
          const daysInMonth = getDaysInMonth(selectedYear, monthIdx);
          const firstDay = getFirstDayOfMonth(selectedYear, monthIdx);
          const monthName = monthNames[monthIdx];
          
          const calendarDays = [];
          for (let i = 0; i < firstDay; i++) {
            calendarDays.push(null);
          }
          for (let i = 1; i <= daysInMonth; i++) {
            calendarDays.push(i);
          }

          const monthEvents = events.filter(event => {
            const eventDate = new Date(event.date);
            let matches = eventDate.getFullYear() === selectedYear && eventDate.getMonth() === monthIdx;
            if (selectedCountry !== 'All Countries') {
              matches = matches && event.country === selectedCountry;
            }
            return matches;
          });

          const today = new Date();
          const isCurrentMonth = today.getFullYear() === selectedYear && today.getMonth() === monthIdx;
          const todayDate = today.getDate();

          return (
            <div key={monthIdx} className="bg-white rounded-[8px] border border-[#e5e7eb] overflow-hidden shadow-sm">
              {/* Red Header Bar */}
              <div className="bg-[#c10007] px-4 py-[11px] flex items-center justify-between">
                <p className="text-[16px] font-bold text-white tracking-[-0.5px] leading-[24px]">
                  {monthName}
                </p>
                <p className="text-[12px] font-medium text-white tracking-[-0.5px] leading-[16px]">
                  {monthEvents.length} event{monthEvents.length !== 1 ? '(s)' : ''}
                </p>
              </div>
              
              {/* Mini Calendar */}
              <div className="p-4">
                {/* Day Headers */}
                <div className="grid grid-cols-7 gap-[2px] mb-2">
                  {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((day, idx) => (
                    <div key={idx} className="h-[15px] flex items-center justify-center">
                      <p className="text-[10px] font-semibold text-[#6b7280] tracking-[-0.5px] leading-[10px]">
                        {day}
                      </p>
                    </div>
                  ))}
                </div>
                
                {/* Calendar Grid */}
                <div className="grid grid-cols-7 gap-[2px]">
                  {calendarDays.map((day, idx) => {
                    if (day === null) {
                      return <div key={idx} className="aspect-square" />;
                    }
                    
                    const hasEvents = monthEvents.some(e => new Date(e.date).getDate() === day);
                    const isTodayCell = isCurrentMonth && day === todayDate;
                    
                    return (
                      <div
                        key={idx}
                        onClick={() => {
                          setCurrentDate(new Date(selectedYear, monthIdx, day));
                          setViewMode('day');
                        }}
                        className={`aspect-square rounded-[4px] flex items-center justify-center cursor-pointer hover:bg-gray-100 transition-colors ${
                          isTodayCell ? 'bg-[#fca5a5]' : ''
                        }`}
                      >
                        <p className={`text-[11px] font-medium tracking-[-0.5px] leading-[11px] ${
                          isTodayCell ? 'text-[#7f1d1d]' : hasEvents ? 'text-[#374151]' : 'text-[#6b7280]'
                        }`}>
                          {day}
                        </p>
                      </div>
                    );
                  })}
                </div>
              </div>

              {/* Event List */}
              <div className="border-t border-[#e5e7eb] px-4 py-3">
                {monthEvents.length === 0 ? (
                  <p className="text-[11px] text-[#9ca3af] tracking-[-0.5px] text-center py-1">
                    No events scheduled
                  </p>
                ) : (
                  <div className="space-y-[6px]">
                    {monthEvents.slice(0, 2).map((event) => (
                      <div
                        key={event.id}
                        onClick={() => handleEventClick(event)}
                        className="h-[23px] rounded-[6px] border px-3 flex items-center gap-[7px] cursor-pointer hover:opacity-80 transition-opacity shadow-sm"
                        style={{
                          backgroundColor: event.bgColor,
                          borderColor: event.borderColor
                        }}
                      >
                        <p className="text-[12px] font-semibold text-[#111827] tracking-[-0.5px] leading-[normal]">
                          {new Date(event.date).getDate()}
                        </p>
                        <span className="text-[12px] leading-[24px]">🎉</span>
                        <p className="text-[12px] font-semibold tracking-[-0.5px] leading-[normal] truncate flex-1" style={{ color: event.color }}>
                          {event.title}
                        </p>
                      </div>
                    ))}
                    {monthEvents.length > 2 && (
                      <p className="text-[11px] text-[#3b82f6] tracking-[-0.5px] cursor-pointer hover:underline">
                        +{monthEvents.length - 2} more event{monthEvents.length - 2 !== 1 ? 's' : ''}
                      </p>
                    )}
                  </div>
                )}
              </div>
            </div>
          );
        })}
      </div>
    );
  };

  return (
    <div className="p-8 bg-[#f9fafb] min-h-screen">
      {/* Page Header */}
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-[30px] font-bold text-[#111827] tracking-[-0.5px] leading-[36px]">
            Event Calendar
          </h1>
          <p className="text-[14px] text-[#6b7280] mt-1 leading-[20px] tracking-[-0.5px]">
            Manage events and view public holidays across all countries
          </p>
        </div>

        <button
          onClick={() => setShowCreateModal(true)}
          className="flex items-center gap-2 px-5 py-3 bg-[#C10007] text-white rounded-[6px] hover:bg-[#a10006] transition-colors h-[44px]"
        >
          <Plus className="w-4 h-4" />
          <span className="text-[14px] font-medium tracking-[-0.5px]">Create Event</span>
        </button>
      </div>

      {/* Controls Bar */}
      <div className="bg-white rounded-[12px] border border-[#e5e7eb] p-6 flex items-center justify-between mb-4">
        {/* View Tabs */}
        <div className="flex items-center gap-2">
          <button
            onClick={() => setViewMode('day')}
            className={`px-5 py-2 rounded-[8px] text-[14px] font-medium transition-colors ${
              viewMode === 'day' ? 'bg-[#C10007] text-white' : 'bg-[#f3f4f6] text-[#374151]'
            }`}
          >
            Day
          </button>
          <button
            onClick={() => setViewMode('week')}
            className={`px-5 py-2 rounded-[8px] text-[14px] font-medium transition-colors ${
              viewMode === 'week' ? 'bg-[#C10007] text-white' : 'bg-[#f3f4f6] text-[#374151]'
            }`}
          >
            Week
          </button>
          <button
            onClick={() => setViewMode('month')}
            className={`px-5 py-2 rounded-[8px] text-[14px] font-medium transition-colors ${
              viewMode === 'month' ? 'bg-[#C10007] text-white' : 'bg-[#f3f4f6] text-[#374151]'
            }`}
          >
            Month
          </button>
          <button
            onClick={() => setViewMode('year')}
            className={`px-5 py-2 rounded-[8px] text-[14px] font-medium transition-colors ${
              viewMode === 'year' ? 'bg-[#C10007] text-white' : 'bg-[#f3f4f6] text-[#374151]'
            }`}
          >
            Year
          </button>
        </div>

        {/* Date Navigation */}
        <div className="flex items-center gap-4">
          <button onClick={() => navigateDate('prev')} className="p-2 hover:bg-gray-100 rounded-lg">
            <ChevronLeft className="w-5 h-5" />
          </button>
          
          {viewMode !== 'year' && (
            <input
              type="date"
              value={currentDate.toISOString().split('T')[0]}
              onChange={(e) => setCurrentDate(new Date(e.target.value))}
              className="px-4 py-2 border border-[#d1d5db] rounded-[8px] text-[14px] font-medium"
            />
          )}

          {viewMode === 'month' && (
            <select
              value={monthNames[currentDate.getMonth()]}
              onChange={(e) => {
                const newDate = new Date(currentDate);
                newDate.setMonth(monthNames.indexOf(e.target.value));
                setCurrentDate(newDate);
              }}
              className="px-4 py-2 border border-[#d1d5db] rounded-[8px] text-[14px] font-medium bg-white"
            >
              {monthNames.map(month => <option key={month}>{month}</option>)}
            </select>
          )}

          <select
            value={viewMode === 'year' ? selectedYear : currentDate.getFullYear()}
            onChange={(e) => {
              if (viewMode === 'year') {
                setSelectedYear(Number(e.target.value));
              } else {
                const newDate = new Date(currentDate);
                newDate.setFullYear(Number(e.target.value));
                setCurrentDate(newDate);
              }
            }}
            className="px-4 py-2 border border-[#d1d5db] rounded-[8px] text-[14px] font-medium bg-white"
          >
            {Array.from({ length: 10 }, (_, i) => 2020 + i).map(year => (
              <option key={year}>{year}</option>
            ))}
          </select>
          
          <button onClick={() => navigateDate('next')} className="p-2 hover:bg-gray-100 rounded-lg">
            <ChevronRight className="w-5 h-5" />
          </button>
        </div>

        {/* Country Filter */}
        <div className="flex items-center gap-2">
          <Globe className="w-5 h-5 text-[#6b7280]" />
          <select
            value={selectedCountry}
            onChange={(e) => setSelectedCountry(e.target.value)}
            className="px-4 py-2 border border-[#d1d5db] rounded-[8px] text-[14px] bg-white"
          >
            <option>All Countries</option>
            <option>Philippines</option>
            <option>UAE</option>
            <option>Singapore</option>
            <option>USA</option>
            <option>Canada</option>
          </select>
        </div>
      </div>

      {/* Calendar Views */}
      {viewMode === 'day' && renderDayView()}
      {viewMode === 'week' && renderWeekView()}
      {viewMode === 'month' && renderMonthView()}
      {viewMode === 'year' && renderYearView()}

      {/* Modals */}
      {showCreateModal && (
        <CreateEventModal 
          onClose={() => {
            setShowCreateModal(false);
            setEditingEvent(null);
          }} 
          onCreateEvent={handleCreateEvent} 
          onEditEvent={handleUpdateEvent}
          editingEvent={editingEvent}
        />
      )}

      {showEventDetail && selectedEvent && (
        <EventDetailModal
          event={selectedEvent}
          onClose={() => {
            setShowEventDetail(false);
            setSelectedEvent(null);
          }}
          onDelete={handleDeleteEvent}
          onEdit={handleEditEvent}
        />
      )}
    </div>
  );
}