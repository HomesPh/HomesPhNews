"""
HomesPh Articles Service - Scheduler
Background job that runs every hour to scrape and process news articles.
Includes URL deduplication to avoid processing same articles.

This is the articles-only scheduler. For the unified service, use root scheduler.py.
"""
...
def send_discord_notification(results: List[Dict]):
    ...
    try:
        ...
        embed = {
            ...
            "footer": {
                "text": f"HomesPh Articles Service • {total_countries} Countries • Next: {next_run_text}",
...
async def run_hourly_job():
    ...
    print("\n" + "=" * 70)
    print(f"🚀 ARTICLES JOB STARTED (FRESH NEWS MODE)")
